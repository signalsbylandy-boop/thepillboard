...
$milestones = @("v0.1 — Foundation", "v0.2 — Auth & Users", "v0.3 — Content Core", "v0.4 — Real-Time Presence", "v0.5 — Moderation", "v0.6 — Frontend Polish", "v0.7 — Performance & Analytics")
...
$issues = @(
    ...
    @{
        ...
        body = "## Goal`nApply the Drizzle schema to D1 locally and remotely.`n`n## Tasks`n- [ ] Run `npm run db:generate` to generate SQL migrations`n- [ ] Run `npm run db:migrate` locally`n- [ ] Verify all tables exist with `wrangler d1 execute pillboard-db --command 'SELECT name FROM sqlite_master'``n- [ ] Add migration to CI deploy-production.yml (already scaffolded)"
        labels = "db,infra,P0"
        milestone = "v0.1 — Foundation"
    },
    @{
    ...
    @{
        ...
        body = "## Goal`nUsers can register with email + password. Password hashed with Web Crypto PBKDF2.`n`n## API`n`POST /auth/register` → 201 `{ user, token, expiresAt }``n`n## Acceptance Criteria`n- [ ] Registration returns JWT`n- [ ] Duplicate email/username returns 409`n- [ ] Password stored as PBKDF2 hash, never plaintext`n- [ ] Session row created in D1"
        labels = "auth,P0"
        milestone = "v0.2 — Auth & Users"
    },
    @{
        title = "[AUTH] Login flow with JWT session management"
        body = "## Goal`nUsers can log in and receive a JWT valid for 7 days.`n`n## API`n`POST /auth/login` → 200 `{ user, token, expiresAt }``n`n## Acceptance Criteria`n- [ ] Invalid credentials return 401`n- [ ] Banned users see 403 message`n- [ ] Token stored in Zustand (persisted to localStorage)`n- [ ] `GET /auth/me` returns current user with valid token"
        labels = "auth,P0"
        milestone = "v0.2 — Auth & Users"
    },
    @{
        title = "[AUTH] Logout + session invalidation"
    ...
    @{
        title = "[MOD] Submission review queue — pending → approved/rejected"
    ...
    @{
        ...
        body = "## Goal`nAuthenticated users can report content. Reports go to `/moderation/reports`.`n`n## API`n`POST /reports` `{ targetId, targetType, reason }``n`n## Acceptance Criteria`n- [ ] One report per user per target`n- [ ] Auto-flag post to moderation queue at 3+ reports`n- [ ] Mods can resolve or dismiss reports"
    ...
    @{
        ...
        body = "## Goal`nFully usable on mobile (320px+).`n`n## Tasks`n- [ ] Sidebar hidden on mobile (≤ lg), shown on desktop`n- [ ] PostCard responsive (thumbnail hidden on small)`n- [ ] Header collapses to icon-only on small screens`n- [ ] Comment form works on mobile"
        labels = "frontend,P1"
        milestone = "v0.6 — Frontend Polish"
    },
    @{
        title = "[FE] Dark / light theme toggle"
        body = "Toggle in header. Persisted to localStorage. Respects system preference on first visit. Applied via `class='dark'` on `<html>`."
        labels = "frontend,P2"
        milestone = "v0.6 — Frontend Polish"
    },
    @{
    ...
    @{
        ...
        body = "## Goal`n`/u/:username` shows user's submissions, comment history, karma, and join date.`n`n## API`n`GET /users/:username` → public profile + paginated posts + comments"
    ...
    @{
        title = "[PERF] D1 query optimization + index audit"
